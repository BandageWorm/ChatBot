...
public class BuildCorpus {
    static void writePCCorpus(String path) throws Exception{
        ...
        BufferedReader brp = new BufferedReader(new FileReader(new File(path+"repos-id-post-cn")));
        BufferedReader brc = new BufferedReader(new FileReader(new File(path+"repos-id-cmnt-cn")));
        String post;
        String cmnt;
        int i = 0;
        while ((post = brp.readLine()) != null && (cmnt = brc.readLine()) != null){
            ...
            post = post.split("\t")[1];
            cmnt = cmnt.split("\t")[1];
            ArrayList<String> postA = Util.segment(post);
            ArrayList<String> cmntA = Util.segment(cmnt);
            postA.addAll(cmntA);
...
//        writeCorpus("D:\\PolyU\\Project\\Text\\wiki.txt");
        writePCCorpus("D:\\PolyU\\Project\\DataSet\\repos\\");
