...
public class MyQuery {
    ...
    public MyQuery() throws Exception{
        this.postSearcher = new IndexSearcher(DirectoryReader.open(Util.getIndexDir("post")));
        this.cmntSearcher = new IndexSearcher(DirectoryReader.open(Util.getIndexDir("cmnt")));
    ...
    private HashMap<Integer, String> Answer(String query, int N) throws Exception {
        ...
        boolean pos = eParser.isPositive(Util.segment(query));
    ...
    private static void testQuery(int N) throws Exception {
        //打印前N个cmnt结果
        DirectoryReader postReader = DirectoryReader.open(Util.getIndexDir("post"));
        DirectoryReader cmntReader = DirectoryReader.open(Util.getIndexDir("cmnt"));
