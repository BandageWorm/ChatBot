...
public class Engine {
    ...
    public Engine() {
        try {
            this.searcher = new IndexSearcher(DirectoryReader.open(Util.getIndexDir("pair")));
    ...
    public ArrayList<String> getW2VWords(String ask) {
        ArrayList<String> inits = Util.segment(ask);
    ...
    double[] sims(String x, String y) {
        ArrayList<String> xArray = Util.segment(x);
        ArrayList<String> yArray = Util.segment(y);
...
//        }
    ...
    public String reply(String ask) {
        ArrayList<String> words = Util.segment(ask);
        ArrayList<Pair> pairs = queryPair(words);
        String res;
        Pair best = pairs.get(0);
        double bstScore = 0;
        for (Pair pair : pairs) {
            if (Util.segment(pair.getCmnt()).size() == 0) continue;
